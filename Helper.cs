...
namespace XKeyboard
{
    public static class Helper
    {
        ...
        public static void Restart()
        {
            ...
            Logger.Log("Helper.Restart(): Creating restart script...");
...
exit";
            ...
            Logger.Log("Helper.Restart(): Restart script created and saved as "+tmpFile);
            System.Diagnostics.Process.Start(tmpFile);
            Logger.Log("Helper.Restart(): Script was invoked, shutting down application.");
        ...
        public static void AddToStartup()
        {
            ...
            try
            {
                Logger.Log("Helper.Startup(): Opening windows registry for write access...");
                var rk = Microsoft.Win32.Registry.CurrentUser.CreateSubKey("SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Run");
                if (rk == null)
                {
                    Logger.Log("Helper.Startup(): Failed to create registry entry. Unknown error.", MessagePriority.High, MessageKind.Error);
                    return;
                }
                if (rk.GetValue(ConfigKeys.StartupKey) != null)
                {
                    Logger.Log("Helper.Startup(): A value with same key already exist, deleting...");
                    rk.DeleteValue(ConfigKeys.StartupKey);
                }
                Logger.Log("Helper.Startup(): Creating the entry...");
                rk.SetValue(ConfigKeys.StartupKey, System.Reflection.Assembly.GetExecutingAssembly().Location);
                Logger.Log("Helper.Startup(): Closing registry...");
                rk.Close();
                Logger.Log("Helper.Startup(): Done. Added to startup. ");
            }
            catch (Exception ex)
            {
        ...
        public static void RemoveFromStartup()
        {
            ...
            try
            {
                ...
                if (rk == null)
                {
                    Logger.Log("Helper.Startup(): The key doesn't exist, ignored...");
                    return;
                }
                if (rk.GetValue(ConfigKeys.StartupKey) != null)
                {
                    Logger.Log("Helper.Startup(): Deleting registry key...");
                    rk.DeleteValue(ConfigKeys.StartupKey);
                }
                Logger.Log("Helper.Startup(): Closing registry...");
                rk.Close();
                Logger.Log("Helper.Startup(): Done. The application was removed from windows startup. ");
            }
            catch (Exception ex)
            {
                Logger.Write("Error: " + ex.Message, MessagePriority.High, MessageKind.Error);
        ...
        public static bool IsStartup()
        {
            try
            {
                Logger.Log("Helper.IsStartup(): Opening windows registry for read-only access...");
                var rk = Microsoft.Win32.Registry.CurrentUser.CreateSubKey("SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Run");
                if (rk == null)
                {
                    Logger.Log("Helper.IsStartup(): Failed to open registry. Unknown error.", MessagePriority.High, MessageKind.Error);
                    return false;
                }
                if (rk.GetValue(ConfigKeys.StartupKey) != null)
                {
                    Logger.Log("Helper.IsStartup(): A value with the key already exist, returning...");
                    return true;
                }
                return false;
            }
